...
{
  software_serial = true;
  soft_nextion->begin(baud);
  flushSerial();
}

Nextion::Nextion(HardwareSerial &next, uint32_t baud) : hard_nextion(&next)
{
  software_serial = false;
  hard_nextion->begin(baud);
  flushSerial();
}
...
boolean Nextion::ack(void)
{
  ...
  if (sizeof(bytes) != serial_readBytes((char *)bytes, sizeof(bytes)))
  {
    return 0;
  } //end if
...
unsigned int Nextion::getComponentValue(String component)
{
  ...
  uint8_t temp[8] = {0};
  serial_setTimeout(20);
  if (sizeof(temp) != serial_readBytes((char *)temp, sizeof(temp)))
  {
    return -1;
  } //end if
...
void Nextion::serial_setTimeout(unsigned long timeout)
{
  if (software_serial)
  {
    soft_nextion->setTimeout(timeout);
  }
  else
  {
    hard_nextion->setTimeout(timeout);
  }
}

size_t Nextion::serial_readBytes(char *buffer, size_t length)
{
  if (software_serial)
  {
    return soft_nextion->readBytes(buffer, length);
  }

  return hard_nextion->readBytes(buffer, length);
}

int Nextion::serial_available(void)
{
  if (software_serial)
  {
    return soft_nextion->available();
  }

  return hard_nextion->available();
}

int Nextion::serial_read(void)
{
  if (software_serial)
  {
    return soft_nextion->read();
  }

  return hard_nextion->read();
}

size_t Nextion::serial_print(const char *x)
{
  if (software_serial)
  {
    return soft_nextion->print(x);
  }

  return hard_nextion->print(x);
}

size_t Nextion::serial_write(int n)
{
  if (software_serial)
  {
    return soft_nextion->write(n);
  }

  return hard_nextion->write(n);
}

void Nextion::serial_flush()
{
  if (software_serial)
  {
    soft_nextion->flush();
