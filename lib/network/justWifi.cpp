...
uint8_t JustWifi::_populate(uint8_t networkCount)
{
    ...
    for (int8_t i = 0; i < networkCount; ++i)
    {
...
#ifdef ESP32
                // Check security
                if ((sec_scan != WIFI_AUTH_OPEN) && (entry->pass == NULL))
                    continue;
#else
                // Check security
                if ((sec_scan != ENC_TYPE_NONE) && (entry->pass == NULL))
                    continue;
#endif
...

        {
            char buffer[128];
            sprintf_P(buffer,
                      PSTR("%s BSSID: %02X:%02X:%02X:%02X:%02X:%02X CH: %2d RSSI: %3d SSID: %s"),
                      (known ? "-->" : "   "),
        ...
        // Connect
        {
            ...
            if (entry.scanned)
            {
                snprintf_P(buffer, sizeof(buffer),
                           PSTR("BSSID: %02X:%02X:%02X:%02X:%02X:%02X CH: %02d, RSSI: %3d, SSID: %s"),
