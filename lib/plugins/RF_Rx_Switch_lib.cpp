...
void RCSwitch::enableReceive(int interrupt, OnRFMessageRecive callback)
{
  RCSwitch_OnReciveCallback = callback;
      ...
      if (repeatCount == 2) {
        for(unsigned int i = 1; i <= numProto; i++) {
          if (receiveProtocol(i, changeCount)) {
            ...
            if(RCSwitch_OnReciveCallback != nullptr)
            {
              RCSwitch_OnReciveCallback(RCSwitch::nReceivedValue);
            }
            break;
          }
        }
